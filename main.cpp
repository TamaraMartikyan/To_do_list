...
struct Task {
    ...
    // For printing the task
    string toString() const {
        char date_str[26];
        ctime_r(&due_date, date_str);
        string date_string(date_str);
        date_string.pop_back(); // Remove newline character
...
class TodoList {
private:
    // Store tasks in a list for efficient insertion/removal
    list<Task> tasks;
    
    // Maps for quick lookup by different properties
    unordered_map<int, list<Task>::iterator> id_map;
    unordered_map<string, vector<list<Task>::iterator>> category_map;
    
    // Vector of iterators for priority queue functionality
...
public:
    ...
    bool loadFromFile(const string& filename) {
        ...
        while (getline(file, line)) {
            ...
            time_t due_date = stol(tokens[3]);
            int priority = stoi(tokens[4]);
            bool completed = stoi(tokens[5]);
            
            tasks.emplace_back(id, description, category, due_date, priority);
            auto task_it = prev(tasks.end());
            task_it->completed = completed;
...
int main() {
    ...
    // Sample tasks
    todoList.addTask("Complete C++ assignment", "School", getFutureDate(7), 2);
    todoList.addTask("Buy groceries", "Personal", getFutureDate(1), 3);
    todoList.addTask("Pay bills", "Finance", getFutureDate(5), 1);
